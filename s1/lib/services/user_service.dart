...
class UserService {
  final CollectionReference<LibriUniUser> _usersCollection = FirebaseFirestore
      .instance
      .collection('users')
      .withConverter<LibriUniUser>(
  ...
  Future<DocumentReference<LibriUniUser>> addUser(LibriUniUser user) async {
    ...
    if (existingUser != null) {
      throw Exception(
        'User with ID String ${user.userIdString} already exists.',
      );
    }
  ...
  Future<LibriUniUser?> getUserByUserIdString(String userIdString) async {
    final querySnapshot =
        await _usersCollection
            .where('userIdString', isEqualTo: userIdString)
            .limit(1)
            .get();
          ...
          users
              .where(
                (user) =>
                    ...
                    user.userIdString.toLowerCase().contains(
                      query.toLowerCase(),
                    ),
              )
              .toList(),
