...
class FineService {
  final CollectionReference<FineModel> _finesCollection = FirebaseFirestore
      .instance
      .collection('fines')
      .withConverter<FineModel>(
  ...
  Future<void> ensureFineRecord({
    ...
    if (loan.returnDate != null || loan.dueDate.compareTo(now) >= 0) {
      ...
      print(
        "Loan ${loan.id} is not currently overdue. Skipping fine record creation/update.",
      );
    ...
    switch (book.tag.toLowerCase()) {
      case 'red':
        ratePerDay = _redTagFineRateFineService;
        break;
      case 'yellow':
        ratePerDay = _yellowTagFineRateFineService;
        break;
      case 'green':
      default:
        ratePerDay = _greenTagFineRateFineService;
        break;
    }
    final fineAmount = daysOverdue * ratePerDay;

    final querySnapshot =
        await _finesCollection
  ...
  Future<void> markFineDocumentAsPaid(
    String fineDocId,
    Timestamp paidDate,
  ) async {
    try {
