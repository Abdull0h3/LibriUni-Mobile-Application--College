...
class _LoanFormScreenState extends State<LoanFormScreen> {
  ...
  Future<void> _searchUser() async {
    ...
    //todo: can???
    _userService
        .searchUsersStream(_userIdStringController.text.trim())
        .listen((users) {
          if (mounted) {
            setState(() {
        ...
        .onError((error) {
          if (mounted) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Error searching users: $error'),
    ...
    if (_formKey.currentState!.validate() && _selectedUser != null) {
      if (widget.book.status != 'Available') {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Book "${widget.book.title}" is not available for loan.',
            ),
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Book "${widget.book.title}" loaned to ${_selectedUser!.name} successfully!',
              ),
        ...
        children: [
          Padding(
            ...
            child: Form(
              key: _formKey,
              child: ListView(
                // Changed to ListView for better scrolling with search results
                children: [
                  Text(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            subtitle: Text(
                              '${user.userIdString} - ${user.email}',
                            ),
                            onTap: () {
                              setState(() {
                                _selectedUser = user;
                                _userIdStringController.text =
                                    "${user.name} (${user.userIdString})"; // Display selected user
                  ...
                  if (_selectedUser != null)
                    Padding(
                      ...
                      child: Chip(
                        ...
                        label: Text(
                          'Selected User: ${_selectedUser!.name} (${_selectedUser!.userIdString})',
                        ),
                        onDeleted: () {
                          setState(() {
                            _selectedUser = null;
                  ...
                  ElevatedButton.icon(
                    ...
                    onPressed:
                        _isLoading || widget.book.status != 'Available'
                            ? null
                            : _submitLoan,
