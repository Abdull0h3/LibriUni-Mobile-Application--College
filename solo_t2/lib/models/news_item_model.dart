...
class NewsItemModel {
  ...
  factory NewsItemModel.fromFirestore(DocumentSnapshot<Map<String, dynamic>> snapshot) {
    final data = snapshot.data();
    if (data == null) {
      throw StateError('Missing data for NewsItemModel from snapshot ${snapshot.id}');
    }
    ...
    return NewsItemModel(
      ...
      fullDetails: data['fullDetails'] ?? data['description'] ?? 'No Details Provided',
  ...
  static NewsPriority _priorityFromString(String priorityStr) {
    return NewsPriority.values.firstWhere(
      (e) => e.toString().split('.').last.toLowerCase() == priorityStr.toLowerCase(),
