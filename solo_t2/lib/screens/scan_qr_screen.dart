...
class _ScanQrScreenState extends State<ScanQrScreen> {
  ...
  Future<void> _processScannedCode(String code) async {
    if (_screenState == ScanScreenState.processing || code == _lastScannedCode) return;
    ...
    Future.delayed(const Duration(seconds: 3), () {
      if (mounted && (_screenState == ScanScreenState.bookNotFound || _screenState == ScanScreenState.error)) {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Book "${_scannedBook!.title}" received successfully!'), backgroundColor: Colors.green),
        );
        _resetToScanning();
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error receiving book: $e'), backgroundColor: Colors.red),
        );
        setState(() => _screenState = ScanScreenState.bookFound); // Revert to previous state
      }
    }
  }
  ...
  void _handleLoanBook() {
    if (_scannedBook != null) {
      Navigator.pushNamed(context, AppRoutes.loanForm, arguments: _scannedBook).then((_) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (_screenState != ScanScreenState.scanning && _screenState != ScanScreenState.processing)
            IconButton(
            ...
            Expanded(
              ...
              child: MobileScanner(
                ...
                onDetect: (capture) {
                  ...
                  if (barcodes.isNotEmpty && barcodes.first.rawValue != null) {
                     if (mounted && (_screenState == ScanScreenState.scanning)) { // Process only if in scanning state
             ...
             Padding(
              ...
              child: Text(
                ...
                style: TextStyle(fontSize: 16, color: AppColors.textColorDark.withOpacity(0.8)),
        ...
        return Center(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Icon(
                  _screenState == ScanScreenState.bookNotFound ? Icons.search_off : Icons.error_outline,
                  color: Colors.red,
                  size: 60,
                ),
                ...
                Text(
                  _errorMessage,
                  textAlign: TextAlign.center,
                  style: const TextStyle(fontSize: 18, color: AppColors.textColorDark),
                ),
  ...
  Widget _buildBookDetailsView() {
    ...
    final bool isBookAvailableForLoan = _scannedBook!.status == 'Available' && _activeLoan == null;
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(_scannedBook!.title, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: AppColors.primaryColor)),
              const SizedBox(height: 8),
              _buildInfoRow('Author:', _scannedBook!.author),
              _buildInfoRow('Code:', _scannedBook!.code),
              _buildInfoRow('Status:', _scannedBook!.status,
                  valueColor: _scannedBook!.status == 'Available' ? Colors.green.shade700 : Colors.orange.shade700),
              if (_scannedBook!.publishedYear != null)
                _buildInfoRow('Year:', _scannedBook!.publishedYear.toString()),
              const Divider(height: 30, thickness: 1),

              if (isBookOnLoan) ...[
                Text('Currently On Loan:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.orange.shade800)),
                _buildInfoRow('Borrower:', _activeLoan!.userName),
                _buildInfoRow('Loan Date:', DateFormat.yMMMd().format(_activeLoan!.loanDate.toDate())),
                _buildInfoRow('Due Date:', DateFormat.yMMMd().format(_activeLoan!.dueDate.toDate()),
                  valueColor: _activeLoan!.isOverdue ? Colors.red.shade700 : null),
                if (_activeLoan!.isOverdue)
                   Padding(
                     padding: const EdgeInsets.only(top: 4.0),
                     child: Text('OVERDUE!', style: TextStyle(color: Colors.red.shade700, fontWeight: FontWeight.bold, fontSize: 16)),
                   ),
                ...
                Text('Book is Available for Loan', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.green.shade800)),
                const SizedBox(height: 25),
                Center(
                  child: ElevatedButton.icon(
                 ...
                 Text('Book Status: ${_scannedBook!.status}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.blueGrey)),
                 const SizedBox(height: 10),
                 Text(
                    _scannedBook!.status == 'Lost' ? 'This book is marked as Lost.' :
                    _scannedBook!.status == 'Maintenance' ? 'This book is currently under maintenance.' :
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(
            label,
            style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textColorDark.withOpacity(0.85), fontSize: 15),
          ),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
              value,
              style: TextStyle(fontSize: 15, color: valueColor ?? AppColors.textColorDark),
            ),
          ),
        ],
      ),
